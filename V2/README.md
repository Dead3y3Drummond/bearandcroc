...
## Resend setup
The PHP endpoint reads the API key from the server environment variable `RESEND_API_KEY` or the private deployment configuration generated by GitHub Actions. Never commit the API key to GitHub or expose it in browser JavaScript.
...
GitHub Actions deploys the contents of `V2/` to the domain-scoped Hostmonster FTP account over explicit FTPS. Configure these repository Actions secrets:
...
The account is rooted at the site's `public_html` directory, so the workflow deploys to `/`.

Run **Actions → Deploy Bear & Croc V2 → Run workflow** for a manual deployment. Later pushes that change files inside `V2/` deploy automatically.
